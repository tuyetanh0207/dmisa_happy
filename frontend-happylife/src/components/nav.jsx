...
const Nav = () => {
  ...
  if (
    user &&
    (user.userInfo.role === "INSUARANCE_MANAGER" ||
      user.userInfo.role === "ACCOUNTANT")
  ) {
    return <></>;
  }
        ...
        {navigationRight.map((item) => (
          <a key={item.name} href={item.href} className="btnSignup">
            {item.name === "Login" ? (
              <button
                className={
                  "bg-button-blue text-white px-4 py-2 rounded items-center mr-[1px] ${isButtonVisible ? '' : 'hidden'}"
                }
              ...
              <button
                className={
                  "bg-button-blue text-white px-4 py-2 rounded items-center mr-[2px] ${isButtonVisible ? 'hidden' : ''}"
                }
              >
                {item.name}
              </button>
